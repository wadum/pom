	...
	def __init__(self, m = None, n = None, values = None):
		...
		if m is not None and n is not None and values is None:
			if n > 0 and m > 0:
				...
				raise ValueError("m og n skal begge være positive tal.")
		if m is not None and n is not None and values is not None:
			if n > 0 and m > 0:
				if n * m == len(values):
					self.matrix = [[values[j*n+i] for i in range(n)] for j in range(m)]
				else:
					raise ValueError("Antallet af værdier i values matcher ikke størrelsen af matricen.")
			else:
				raise ValueError("m og n skal begge være positive tal.")
...
if __name__ == '__main__':
	...
	B = SimpleMatrix(3,2,[5,4,3,2,1,0])
	E = SimpleMatrix(3,3,[1,0,0,0,1,0,0,0,1])
	Z = SimpleMatrix(3,3)
	a = SimpleMatrix(3,3,[0,-1,1,-1,1,-1,0,-1,0])
	b = SimpleMatrix(3,3,[-1,-1,0,0,0,-1,1,0,-1])
